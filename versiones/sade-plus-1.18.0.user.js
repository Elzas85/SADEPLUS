...
// @namespace    local
// @version      1.18.0
// @description  Vista externa del Expediente Electrónico y de GEDO. Replica sus listados en una ventana propia, con texto completo, orden, búsqueda, etiquetas y anotaciones, y abre en una ventana nueva el inicio de un documento GEDO o de una nota CCOO.
// @match        https://eue-pr.apps.buenosaires.gob.ar/expedientes-web/*
// @match        https://eut-pr.apps.buenosaires.gob.ar/gedo-web/*
// @match        https://euc-pr.apps.buenosaires.gob.ar/ccoo-web/*
 ...
 *
 * ----------------------------------------------------------------------
 * INICIAR UN DOCUMENTO EN GEDO O UNA NOTA EN CCOO
 *
 * Dos botones de la barra, visibles en el Expediente Electrónico y en
 * GEDO, abren en una pestaña nueva el módulo que corresponde y accionan
 * allí su botón "Inicio de Documento", que en GEDO y en CCOO se llama
 * igual. El sistema abre entonces su ventana "Iniciar Producción de
 * Documento" y la herramienta se detiene: no completa campos, no elige el
 * tipo de documento y no confirma nada.
 *
 * El rótulo del botón se verifica dos veces antes de accionarlo, como con
 * Tramitar. Si el módulo pidiera ingresar de nuevo, el botón no aparece: se
 * informa y no se acciona nada.
 *
 * ----------------------------------------------------------------------
 * SEGURIDAD
 *
 *   USUARIO      Etiquetas, anotaciones y contraseña de las copias se
 *                guardan separadas por usuario del sistema, que se lee del
 *                encabezado de cada módulo. Sin usuario identificado no se
 *                muestra ni se guarda nada de eso. La barra de título
 *                indica con qué usuario se está trabajando.
 *   DOS PESTAÑAS Cada cambio vuelve a leer lo guardado y modifica sólo la
 *                fila en cuestión. Si dos pestañas escriben la misma
 *                anotación, se conservan los dos textos.
 *   COPIAS       El archivo exportado tiene formato propio (.sadeplus) y va
 *                cifrado con una contraseña que se pone una sola vez en
 *                cada computadora (AES-GCM, clave derivada con PBKDF2).
 *   ENCARGOS     Lo que se hace en una pestaña nueva queda asignado a esa
 *                pestaña por un identificador al azar, con su usuario y su
 *                módulo de destino. Cada etapa se asienta antes de
 *                accionar, y nada se repite de manera automática.
 *   TRAMITAR     Un doble clic no abre dos veces la misma actuación, y se
 *                comprueba que el expediente haya aparecido.
 *   SESIÓN       Mientras hay actividad, se evita que la sesión del sistema
 *                venza; con el equipo desatendido vence como siempre. La
 *                barra de título tiene un botón para cerrarla al terminar.
 *   MARCOS       La herramienta no se ejecuta dentro de marcos ni sobre
 *                documentos que no sean páginas.
 *
 * ----------------------------------------------------------------------
 * REQUISITO
 *
...
(function () {
    ...
    // Nunca dentro de un marco: el sistema usa marcos para visores y
    // ventanas auxiliares, y ahí la herramienta no tiene nada que hacer.
    // Tampoco en respuestas que no sean una página, como un PDF.
    if (window.top !== window.self) return;
    if (document.contentType && !/html/i.test(document.contentType)) return;

    const VERSION = '1.18.0';
    const CLAVE = 'sade.plus.v1';
    // Clave de las versiones anteriores para el encargo de la pestaña nueva.
    // Ya no se usa: los encargos van ahora al almacén de Tampermonkey, cada
    // uno con su identificador. Sólo se borra si quedó alguno viejo.
    const ENCARGO_VIEJO = 'sade.plus.abrir';

    // Módulo en el que se está. CCOO sólo interesa para completar el inicio
    // de una nota encargado desde otra ventana: ahí la herramienta no dibuja
    // nada más.
    const EN_GEDO = /\/gedo-web\//.test(location.pathname);
    const EN_CCOO = /\/ccoo-web\//.test(location.pathname);
    const EN_EE = /\/expedientes-web\//.test(location.pathname);
    ...
    //
    // El almacén de Tampermonkey pertenece al navegador y no a la sesión del
    // sistema, y además lo comparten las ventanas de incógnito. Sin una
    // separación, si en esta computadora ingresara otra persona con su propio
    // usuario vería las etiquetas y las anotaciones del anterior. Por eso todo
    // lo que es del usuario se guarda separado por cuenta y, si la cuenta no
    // puede identificarse, no se lee ni se escribe nada de lo guardado.
    //
    // La cuenta se lee del encabezado común de los módulos, arriba a la
    // derecha, donde el sistema muestra el nombre de usuario. Se mira
    // únicamente ese encabezado, porque en el resto de la página figuran
    // usuarios de otras personas (Usuario anterior, Enviado por). Si aparecen
    // dos nombres distintos no se elige ninguno. Y la cuenta no se recuerda
    // entre páginas: sale siempre de la página a la vista, para que la de una
    // sesión cerrada no quede asociada a la pestaña.

    const RE_USUARIO = /^[A-Z][A-Z0-9._-]{2,39}$/;

    function usuarioEnPagina() {
        const vistos = new Set();
        for (const cab of document.querySelectorAll('.module-header')) {
            for (const el of cab.querySelectorAll('.texto-header-unificado')) {
                if (el.children.length) continue;
                const t = (el.textContent || '').trim();
                if (RE_USUARIO.test(t)) vistos.add(t);
            }
        }
    ...
    const SIN_CUENTA = 'No fue posible identificar con qué usuario se ingresó al sistema. ' +
        'El listado puede consultarse igual, pero no se muestran ni se guardan etiquetas ni ' +
        'anotaciones, para no mezclar los datos de dos usuarios. Recargue la página; si el aviso ' +
        'persiste, informe en qué pantalla ocurre.';

    // ===================== ALMACÉN =====================
    //
    // El Expediente Electrónico, GEDO y CCOO están en subdominios distintos y
    // no comparten localStorage. Lo que tiene que verse en más de un módulo
    // (etiquetas, anotaciones, contraseña del respaldo, encargos entre
    // ventanas) va al almacén de Tampermonkey, que sí es común a todos los
    // @match del script. Si no estuviera disponible se usa localStorage, y
    // entonces cada módulo tiene lo suyo.

    const GM_OK = (typeof GM_getValue === 'function' && typeof GM_setValue === 'function');

    function leerAlmacen(k) {
        try {
            const t = GM_OK ? GM_getValue(k, null) : localStorage.getItem(k);
            return t ? JSON.parse(t) : null;
        } catch { return null; }
    }

    function escribirAlmacen(k, v) {
        try {
    ...
    // Aviso de cambios hechos desde otra pestaña, para redibujar con lo
    // último guardado. Tampermonkey lo informa con GM_addValueChangeListener;
    // sin él, el navegador lo informa con el evento storage.
    const oyentes = new Set();
    function escucharAlmacen(k, fn) {
        if (oyentes.has(k)) return;
        oyentes.add(k);
        if (GM_OK && typeof GM_addValueChangeListener === 'function') {
            try {
                GM_addValueChangeListener(k, (nombre, antes, ahora, remoto) => { if (remoto) fn(); });
                return;
            } catch { /* se sigue con el evento del navegador */ }
        }
        addEventListener('storage', e => { if (e.key === k) fn(); });
    }
    ...
    // navegador, indexados por número de expediente y separados por usuario,
    // y se llevan de una computadora a otra con exportar e importar.
    //
    // Se llaman ANOTACIONES y no notas, para no confundirlas con la nota
    // electrónica, que es un acto administrativo.
    //
    // Cada cambio vuelve a leer lo guardado antes de escribir y modifica
    // únicamente la fila en cuestión. Así, dos pestañas abiertas a la vez no
    // se pisan: si cada una anota una actuación distinta, quedan las dos
    // anotaciones. Y si las dos escribieron sobre la misma, se conservan
    // ambos textos con una separación visible, en lugar de perder uno.

    const CLAVE_MARCAS = 'sade.plus.marcas.v1';
    const CLAVE_MIGRADO = 'sade.plus.migrado.v1';
    const SEPARADOR_NOTA = '—— texto guardado desde otra pestaña ——';
    ...
    const claveMarcas = () => CLAVE_MARCAS + '@' + USUARIO;

    // Lo guardado por las versiones anteriores, que no separaban por
    // usuario. Se busca en el orden en que fueron cambiando las claves.
    function marcasSinUsuario() {
        const candidatos = [leerAlmacen(CLAVE_MARCAS), leerAlmacen(CLAVE_MARCAS_VIEJA),
                            leerLocal(CLAVE_MARCAS, CLAVE_MARCAS_VIEJA)];
        return candidatos.find(m => m && typeof m === 'object' &&
            (Object.keys(m.filas || {}).length || (m.etiquetas || []).length)) || null;
    }

    // Lo que había guardado sin usuario pasa, una sola vez, al primer usuario
    // que se identifica en este navegador. Una sola vez y no una por
    // usuario: si no, cualquier otra persona que ingresara después con su
    // propio usuario recibiría las etiquetas y anotaciones del anterior. La
    // copia anterior no se borra: queda en el almacén, sin uso, como
    // resguardo, y la marca de migración registra a qué usuario fue.
    function migrarSiHaceFalta() {
        if (!USUARIO || leerAlmacen(CLAVE_MIGRADO)) return;
        const viejo = marcasSinUsuario();
        if (viejo && !leerAlmacen(claveMarcas())) escribirAlmacen(claveMarcas(), normalizarMarcas(viejo));
        escribirAlmacen(CLAVE_MIGRADO, { usuario: USUARIO, fecha: Date.now(), version: VERSION, habia: !!viejo });
    }

    let MARCAS = normalizarMarcas(null);

    // Vuelve a leer lo guardado. Sin usuario identificado no hay nada que
    // mostrar.
    function refrescarMarcas() {
        MARCAS = USUARIO ? normalizarMarcas(leerAlmacen(claveMarcas())) : normalizarMarcas(null);
    }

    const guardarMarcas = () => {
        if (!USUARIO) { avisar(SIN_CUENTA); return false; }
        if (escribirAlmacen(claveMarcas(), MARCAS)) return true;
        avisar('No se pudieron guardar las etiquetas y las anotaciones: el almacén del navegador no admite más datos.');
        return false;
    };
    ...
    // Aplica un cambio a una sola fila, sobre lo último guardado.
    //
    // cambio.et    las etiquetas de la fila, completas
    // cambio.nota  el texto de la anotación
    // cambio.base  el texto que tenía la anotación cuando se empezó a
    //              escribir. Si lo guardado ya no coincide con él, otra
    //              pestaña lo modificó mientras tanto: no se pisa.
    //
    // Devuelve { ok, combinada }: combinada indica que se conservaron los dos
    // textos.
    function fijarMarca(clave, cambio) {
        if (!USUARIO) { avisar(SIN_CUENTA); return { ok: false }; }
        refrescarMarcas();
        const previa = MARCAS.filas[clave] || {};
        const m = { et: (previa.et || []).slice(), nota: String(previa.nota || '') };
        let combinada = false;

        if ('et' in cambio) m.et = cambio.et.slice();
        if ('nota' in cambio) {
            const nueva = String(cambio.nota || '');
            const otra = ('base' in cambio) && String(cambio.base || '') !== m.nota &&
                         m.nota.trim() && m.nota.trim() !== nueva.trim();
            if (!otra) m.nota = nueva;
            else if (nueva.trim()) {
                m.nota = nueva.replace(/\s+$/, '') + '\n\n' + SEPARADOR_NOTA + '\n' + m.nota;
                combinada = true;
            } else {
                // Se pidió borrar, pero otra pestaña escribió algo nuevo: ese
                // texto se conserva.
                combinada = true;
            }
        }
        ...
        else MARCAS.filas[clave] = { et: m.et, nota: m.nota, t: Date.now() };
        return { ok: guardarMarcas(), combinada };
    }
    ...
    function alternarEtiqueta(clave, id) {
        ...
        return fijarMarca(clave, { et });
    }

    function crearEtiqueta(nombre, color) {
        if (!USUARIO) { avisar(SIN_CUENTA); return null; }
        ...
        return guardarMarcas() ? id : null;
    }

    function borrarEtiqueta(id) {
        if (!USUARIO) { avisar(SIN_CUENTA); return; }
        refrescarMarcas();
    ...
    // Identifica al usuario de la página y, si cambió, carga lo suyo.
    // Devuelve true cuando hubo cambio.
    function identificarUsuario() {
        const u = usuarioEnPagina();
        if (u === USUARIO) return false;
        USUARIO = u;
        if (USUARIO) {
            migrarSiHaceFalta();
            escucharAlmacen(claveMarcas(), alCambiarAfuera);
        }
        refrescarMarcas();
        return true;
    }

    // Otra pestaña guardó etiquetas o anotaciones. Se redibuja con lo nuevo,
    // salvo que se esté escribiendo: en ese caso el guardado propio ya
    // vuelve a leer y combina.
    function alCambiarAfuera() {
        if (editando()) return;
        refrescarMarcas();
        if (!win) return;
        const datosAbierto = win.querySelector('.sadeplus-datos').style.display !== 'none';
        if (datosAbierto) panel('datos');
        else if (fichaDe) verFicha(fichaDe);
        else pintar();
    }
    ...
    // Con soloVerificar, ubica la fila y la opción sin accionar nada.
    function tramitar(clave, soloVerificar) {
        const G = grilla();
        if (!G) return { ok: false, msg: 'No se encuentra el listado original.' };
        ...
        if (soloVerificar) return { ok: true };
        item.click();
        return { ok: true };
    }
    ...
    // Aviso en la barra de la ventana principal.
    function avisar(txt, seg) {
        const av = win && win.querySelector('.sadeplus-aviso');
        if (!av || win.style.display === 'none') { cartel(txt, seg ? 'ok' : 'error', seg); return; }
        av.style.display = ''; av.textContent = txt;
        if (seg) setTimeout(() => { av.style.display = 'none'; }, seg * 1000);
    }

    // Aviso suelto, para cuando la ventana principal no está a la vista: en
    // la pestaña que cumple un encargo, y en CCOO, donde la herramienta no
    // dibuja nada más. Lleva el nombre de la herramienta, para que no se
    // confunda con un mensaje del sistema.
    let cartelEl = null, cartelReloj = 0;
    function cartel(txt, tipo, seg) {
        if (!document.body) return;
        if (!cartelEl) {
            cartelEl = document.createElement('div');
            cartelEl.id = 'sadeplus-cartel';
            cartelEl.innerHTML = '<b>SADE+</b><span></span><button title="Cerrar este aviso">×</button>';
            cartelEl.querySelector('button').addEventListener('click', () => { cartelEl.style.display = 'none'; });
            document.body.appendChild(cartelEl);
        }
        clearTimeout(cartelReloj);
        cartelEl.className = tipo || '';
        cartelEl.querySelector('span').textContent = txt;
        cartelEl.style.display = 'flex';
        if (seg) cartelReloj = setTimeout(() => { cartelEl.style.display = 'none'; }, seg * 1000);
    }

    // ===================== ENCARGOS ENTRE PESTAÑAS =====================
    //
    // Hay tres cosas que la herramienta hace en una pestaña nueva, para no
    // perder la que está a la vista:
    //
    //   tramitar  abrir una actuación del buzón, porque Tramitar cambia la
    //             pantalla y se perdería el listado;
    //   gedo      abrir el formulario de inicio de un documento en GEDO;
    //   nota      abrir el formulario de inicio de una nota en CCOO.
    //
    // La pestaña de origen deja anotado un encargo y abre la nueva
    // directamente en el módulo donde está lo que se pidió. Comprobado en la
    // sesión real: con la sesión abierta, el módulo no vuelve a pedir la
    // clave. Si igual la pidiera, la pestaña nueva muestra la pantalla de
    // ingreso del sistema, el encargo no encuentra el botón y lo informa sin
    // accionar nada.
    //
    // Cada encargo lleva un identificador al azar que viaja en el nombre de
    // la ventana nueva (window.name), y el navegador lo conserva cuando la
    // pestaña pasa de un módulo a otro dentro del mismo sitio. Así el encargo
    // pertenece a esa pestaña y a ninguna otra: otra pestaña de la
    // herramienta que se esté cargando al mismo tiempo no lo toma.
    //
    // El encargo registra además el usuario, el módulo de destino y la etapa
    // en la que está. La pestaña nueva lo cumple sólo si coinciden el
    // usuario y el módulo, y asienta cada etapa ANTES de accionar: si la
    // página se recarga en medio, no repite un paso ya dado. Nada se
    // reintenta de manera automática. Y la herramienta se limita a abrir el
    // formulario: no completa campos ni confirma nada.
    //
    // Si el navegador bloquea la pestaña nueva, el encargo se retira en el
    // acto.

    const CLAVE_ENCARGOS = 'sade.plus.encargos.v1';
    const PREFIJO_VENTANA = 'sadeplus-encargo-';
    const PLAZO_INICIO = 90 * 1000;         // para que la pestaña nueva lo tome
    const PLAZO_CONTROL = 25 * 1000;        // para que aparezca el control a accionar
    const PLAZO_RESULTADO = 30 * 1000;      // para que se vea el resultado
    const VIDA_ENCARGO = 15 * 60 * 1000;    // después, se descarta del almacén

    const MODULO = EN_GEDO ? 'gedo' : EN_CCOO ? 'ccoo' : EN_EE ? 'ee' : '';
    const NOMBRE_MODULO = { ee: 'el Expediente Electrónico', gedo: 'GEDO', ccoo: 'CCOO' };

    // Las direcciones de los tres módulos, las mismas sobre las que corre la
    // herramienta. Comprobado en la sesión real: abierto en una pestaña
    // nueva, el módulo no vuelve a pedir la clave mientras haya sesión.
    const DIRECCION = {
    ...
    const TIPOS_ENCARGO = {
        tramitar: { destino: 'ee', que: 'la actuación' },
        gedo: { destino: 'gedo', que: 'el inicio de un documento en GEDO' },
        nota: { destino: 'ccoo', que: 'el inicio de una nota en CCOO' }
    };

    const nuevoId = () => [...crypto.getRandomValues(new Uint8Array(12))]
        .map(x => x.toString(16).padStart(2, '0')).join('');

    function leerEncargos() {
        const e = leerAlmacen(CLAVE_ENCARGOS);
        const out = (e && typeof e === 'object') ? e : {};
        for (const k of Object.keys(out)) {
            if (!out[k] || Date.now() - (out[k].creado || 0) > VIDA_ENCARGO) delete out[k];
        }
        return out;
    }

    // Cada escritura vuelve a leer y toca sólo su encargo, para no pisar los
    // de otras pestañas.
    function fijarEncargo(id, cambio) {
        const e = leerEncargos();
        if (cambio === null) delete e[id];
        else e[id] = Object.assign({}, e[id] || {}, cambio);
        return escribirAlmacen(CLAVE_ENCARGOS, e);
    }

    const retirarEncargo = (id) => fijarEncargo(id, null);

    // Pasos en curso en esta pestaña, para no lanzar dos veces lo mismo por
    // un doble clic.
    const lanzados = {};

    // Deja el encargo y abre la pestaña nueva. Devuelve true si la pestaña
    // se abrió.
    function lanzarEncargo(tipo, extra) {
        const firma = tipo + ':' + ((extra && extra.clave) || '');
        if (Date.now() - (lanzados[firma] || 0) < 8000) {
            avisar('Ya se pidió abrir ' + TIPOS_ENCARGO[tipo].que + ' hace instantes. Se espera a esa pestaña.', 6);
            return false;
        }
        const destino = TIPOS_ENCARGO[tipo].destino;
        const direccion = (destino === MODULO) ? location.origin + location.pathname.replace(/^(\/[^/]+\/).*/, '$1') : DIRECCION[destino];
        if (!direccion) return false;
        const id = nuevoId();
        const ok = fijarEncargo(id, Object.assign({
            id, tipo, usuario: USUARIO, origen: MODULO,
            destino: TIPOS_ENCARGO[tipo].destino,
            etapa: 'pendiente', creado: Date.now(), etapaTs: Date.now()
        }, extra || {}));
        if (!ok) { avisar('No se pudo dejar el encargo para la pestaña nueva: el almacén no está disponible.'); return false; }

        let w = null;
        try { w = window.open(direccion, PREFIJO_VENTANA + id); } catch { w = null; }
        if (!w) {
            retirarEncargo(id);
            avisar('El navegador no permitió abrir la pestaña nueva. Habilite las ventanas emergentes para este sitio.');
            return false;
        }
        lanzados[firma] = Date.now();
        return true;
    }

    // El encargo de esta pestaña, si lo tiene.
    function encargoDeVentana() {
        const n = String(window.name || '');
        if (!n.startsWith(PREFIJO_VENTANA)) return null;
        const id = n.slice(PREFIJO_VENTANA.length);
        const e = leerEncargos()[id];
        if (!e) { window.name = ''; return null; }
        return e;
    }

    // Cierra el encargo: lo retira del almacén y libera el nombre de la
    // ventana, para que una recarga posterior no lo vuelva a tomar.
    function cerrarEncargo(e, txt, tipo) {
        retirarEncargo(e.id);
        window.name = '';
        if (txt) cartel(txt, tipo || 'error', tipo === 'ok' ? 8 : 0);
    }

    // Espera a que una condición se cumpla, consultándola cada tanto.
    function esperar(fn, plazo, cada) {
        return new Promise(res => {
            ...
            const paso = () => {
                ...
                setTimeout(paso, cada || 600);
            };
            paso();
        });
    }

    // Controles del sistema que se accionan, siempre por su rótulo exacto.
    const botonesVisibles = () => [...document.querySelectorAll(
        'button, .z-button, .z-toolbarbutton, a, [role="button"], .z-menuitem, .z-menu-item')]
        .filter(el => vis(el) && !el.closest('#sadeplus, #sadeplus-acerca, #sadeplus-cartel'));

    const rotuloDe = (el) => norm((el.innerText || el.textContent || '').trim());
    const tituloDe = (el) => norm(el.getAttribute('title') || '');

    // El botón que abre el formulario de inicio. Se llama igual en GEDO y en
    // CCOO: "Inicio de Documento" (relevado en la sesión real el 17/09/2026).
    const RE_INICIO = /^inicio de documento$/;

    function controlInicio() {
        return botonesVisibles().find(el => RE_INICIO.test(rotuloDe(el))) || null;
    }

    // La ventana que el sistema abre al accionarlo. En GEDO se titula
    // "Iniciar Producción de Documento". Se acepta cualquier ventana propia
    // del sistema que aparezca después, y el aviso dice cómo se llama: dar
    // por abierto sólo un título exacto haría fallar el aviso si el sistema
    // lo cambia, y lo que importa es que el formulario esté a la vista.
    function formularioAbierto() {
        const w = [...document.querySelectorAll('.z-window-modal, .z-window-highlighted, .z-window-overlapped')]
            .filter(vis)
            .find(x => (x.innerText || '').trim().length > 20);
        if (!w) return null;
        const cab = w.querySelector('.z-window-header, .z-window-modal-header, .z-caption');
        return { ventana: w, titulo: cab ? (cab.innerText || '').trim() : '' };
    }

    // Cumple el encargo de esta pestaña, si lo tiene. Devuelve true cuando
    // la pestaña se abrió para un encargo, y entonces la ventana principal
    // no se despliega: lo que corresponde ver acá es el sistema.
    function cumplirEncargo() {
        const e = encargoDeVentana();
        if (!e || !TIPOS_ENCARGO[e.tipo]) return false;
        const que = TIPOS_ENCARGO[e.tipo].que;
        if (Date.now() - (e.etapaTs || 0) > PLAZO_INICIO) {
            cerrarEncargo(e, 'El pedido de abrir ' + que + ' venció antes de poder cumplirse. No se hizo nada.');
            return true;
        }
        cumplirEncargoEnCurso(e, que);
        return true;
    }

    async function cumplirEncargoEnCurso(e, que) {
        cartel('Abriendo ' + que + '…', 'curso');

        // El usuario de esta pestaña tiene que ser el que pidió el encargo.
        if (e.usuario) {
            const u = await esperar(() => usuarioEnPagina(), 20000);
            if (u && u !== e.usuario) {
                cerrarEncargo(e, 'Esta pestaña tiene la sesión de otro usuario (' + u + '). No se abrió ' + que + '.');
                return;
            }
        }

        // Un encargo nunca se cumple en un módulo que no es el suyo: una
        // pantalla de GEDO no ejecuta un pedido del Expediente Electrónico.
        if (MODULO !== e.destino) {
            cerrarEncargo(e, 'Esta pestaña está en ' + NOMBRE_MODULO[MODULO] + ' y el pedido era para ' +
                NOMBRE_MODULO[e.destino] + '. No se hizo nada.');
            return;
        }

        if (e.etapa === 'accionado') {
            cerrarEncargo(e, 'La página se recargó después de pedir ' + que + ' y no puede confirmarse ' +
                'que se haya abierto. No se repite de manera automática.');
            return;
        }

        if (e.tipo === 'tramitar') { await cumplirTramitar(e); return; }

        const control = await esperar(() => controlInicio(), PLAZO_CONTROL);
        if (!control) {
            cerrarEncargo(e, 'No se encontró en ' + NOMBRE_MODULO[e.destino] + ' el botón "Inicio de Documento". ' +
                'Puede ser que el sistema haya pedido ingresar de nuevo en esa pestaña. No se hizo nada.');
            return;
        }
        // Segunda comprobación del rótulo, inmediatamente antes de accionar.
        if (!RE_INICIO.test(rotuloDe(control))) {
            cerrarEncargo(e, 'El botón no dice lo esperado. Operación cancelada.');
            return;
        }
        fijarEncargo(e.id, { etapa: 'accionado', etapaTs: Date.now() });
        control.click();
        const hecho = await esperar(() => formularioAbierto(), PLAZO_RESULTADO);
        if (hecho) {
            cerrarEncargo(e, 'Listo: el sistema abrió ' + (hecho.titulo ? '"' + hecho.titulo + '"' : 'el formulario') +
                '. Los datos los completa usted.', 'ok');
        } else {
            cerrarEncargo(e, 'Se accionó "Inicio de Documento" en ' + NOMBRE_MODULO[e.destino] + ', pero no pudo ' +
                'confirmarse que el formulario se haya abierto. Verifique la pantalla; no se repite de manera automática.');
        }
    }

    async function cumplirTramitar(e) {
        // El listado puede tardar en aparecer.
        let r = { ok: false, msg: 'No se encuentra el listado original.' };
        const fila = await esperar(() => { r = tramitar(e.clave, true); return r.ok ? r : null; }, 20000, 900);
        if (!fila) {
            cerrarEncargo(e, 'No fue posible abrir ' + e.clave + '. ' + r.msg);
            abrir();
            return;
        }
        fijarEncargo(e.id, { etapa: 'accionado', etapaTs: Date.now() });
        r = tramitar(e.clave);
        if (!r.ok) { cerrarEncargo(e, 'No fue posible abrir ' + e.clave + '. ' + r.msg); abrir(); return; }
        const docs = await esperar(() => grillaDocs(), 40000, 800);
        if (docs) cerrarEncargo(e, '', 'ok');
        else cerrarEncargo(e, 'Se accionó Tramitar sobre ' + e.clave + ', pero no pudo confirmarse que el ' +
            'expediente se haya abierto. Verifique la pantalla; no se repite de manera automática.');
        if (cartelEl && docs) cartelEl.style.display = 'none';
        abrir();
    }

    // Abre una actuación del buzón. Con la opción de pestaña nueva, la deja
    // encargada; si no, o si el navegador bloquea la pestaña, la abre aquí.
    // La búsqueda que sirvió para encontrarla ya cumplió su función y se
    // borra. Se redibuja antes de avisar, porque el redibujado oculta el
    // aviso.
    const tramitesEnCurso = {};

    function abrirActuacion(clave, boton) {
        const borrarBusqueda = () => { ponerBusqueda(''); pagina = 1; pintar(); };
        if (Date.now() - (tramitesEnCurso[clave] || 0) < 15000) {
            avisar('Ya se pidió abrir ' + clave + ' hace instantes. Se espera a que el sistema responda.', 6);
            return;
        }
        if (boton) { boton.disabled = true; setTimeout(() => { boton.disabled = false; }, 5000); }

        if (CFG.enPestanaNueva) {
            if (lanzarEncargo('tramitar', { clave })) {
                tramitesEnCurso[clave] = Date.now();
                borrarBusqueda();
        ...
        if (!r.ok) { avisar(r.msg); return; }
        tramitesEnCurso[clave] = Date.now();
        borrarBusqueda(); win.classList.add('sadeplus-min'); plegado(true);
        // Se comprueba que el expediente haya aparecido. No se reintenta:
        // Tramitar es una orden sobre el sistema.
        esperar(() => grillaDocs(), 20000, 800).then(g => {
            if (!g) avisar('No se pudo confirmar que Tramitar haya sido ejecutado sobre ' + clave + '. Verifique la pantalla.');
        });
    }

    // Botones de la barra: inicio de un documento en GEDO y de una nota en
    // CCOO, cada uno en una pestaña nueva.
    function iniciarEnPestana(tipo, boton) {
        if (boton) { boton.disabled = true; setTimeout(() => { boton.disabled = false; }, 5000); }
        if (lanzarEncargo(tipo)) {
            avisar('Se abre ' + TIPOS_ENCARGO[tipo].que + ' en una pestaña nueva. La herramienta sólo abre el formulario: no completa ni confirma nada.', 8);
        }
    }

    // ===================== LA SESIÓN DEL SISTEMA =====================
    //
    // El sistema cierra la sesión por inactividad. Mientras se trabaja con la
    // herramienta, que lee lo que ya está en pantalla sin consultar al
    // servidor, la sesión puede vencer aunque la persona esté trabajando.
    // Por eso, mientras hay alguien trabajando, se le avisa al servidor que
    // la página sigue en uso, cada CADA_LATIDO.
    //
    // La condición es que haya habido actividad real en los últimos
    // LATIDO_SI_ACTIVO (clic, tecla, rueda del mouse o toque), que la
    // pestaña esté a la vista y que el usuario esté identificado. Con el
    // equipo desatendido el aviso se detiene y la sesión vence como siempre:
    // una sesión que no vence nunca queda a disposición de cualquiera que se
    // siente frente a la computadora.
    //
    // El aviso es el mismo pedido sin efectos que el propio ZK, la
    // biblioteca sobre la que está hecho el sistema, envía para ese fin
    // (comando "dummy", marcado como ignorable). No lee ni escribe datos.
    ...
    const hayQueMantenerViva = () => activoHacePoco();
    const convieneLatir = () => !EN_CCOO && !!USUARIO && !document.hidden && !sesionVencida && hayQueMantenerViva();

    // La página del sistema, para llegar a ZK. Tampermonkey la expone como
    // unsafeWindow; sólo se la usa para leer el escritorio de ZK.
    const paginaSistema = () => (typeof unsafeWindow !== 'undefined' && unsafeWindow) ? unsafeWindow : window;

    async function tocarSesion() {
        const P = paginaSistema();
        const z = P.zk;
        const dt = z && z.Desktop && typeof z.Desktop.$ === 'function' ? z.Desktop.$() : null;
        if (!dt || !dt.id || typeof z.ajaxURI !== 'function') return false;
        const url = z.ajaxURI(null, { desktop: dt, au: true });
        const cuerpo = new URLSearchParams({ dtid: dt.id, cmd_0: 'dummy', opt_0: 'i' });
        try {
            ...
            if (r.body && typeof r.body.cancel === 'function') { try { r.body.cancel(); } catch { /* ya cerrado */ } }
    ...
    function iniciarLatido() {
        for (const ev of ['mousedown', 'keydown', 'wheel', 'touchstart']) {
            addEventListener(ev, () => { ultimaActividad = Date.now(); }, { capture: true, passive: true });
        }
        setInterval(latirSesion, CADA_LATIDO);
    }

    // Cerrar la sesión del sistema al terminar, con el control de salida del
    // propio sistema, que está en su encabezado. No pide confirmación:
    // cerrar la sesión no borra nada. Lo que sí se protege es lo que está a
    // medio hacer: una anotación sin guardar se guarda antes, y si hay una
    // pestaña abierta hace instantes para un encargo, no se sale.
    function controlSalir() {
        for (const cab of document.querySelectorAll('.module-header')) {
            const el = [...cab.querySelectorAll('[title]')]
                .find(x => /^(salir|cerrar sesion)$/.test(tituloDe(x)));
            if (el) return el;
        }
        return null;
    }
    ...
    function cerrarSesion() {
        const salir = controlSalir();
        if (!salir) { avisar('No se encuentra en el encabezado del sistema el control para salir. No se cerró la sesión.'); return; }
        if (win) win.querySelectorAll('[data-marca]').forEach(guardarPendiente);
        const recientes = Object.values(leerEncargos())
            .filter(e => Date.now() - (e.etapaTs || e.creado || 0) < 2 * 60 * 1000);
        if (recientes.length) {
            avisar('Hay una pestaña abierta hace instantes para ' + TIPOS_ENCARGO[recientes[0].tipo].que +
                   ' que todavía no terminó. Espere a que termine antes de cerrar la sesión.', 8);
            return;
        }
        if (tituloDe(salir) !== 'salir' && tituloDe(salir) !== 'cerrar sesion') {
            avisar('El control de salida no dice lo esperado. No se cerró la sesión.');
            return;
        }
        cerrar();
        salir.click();
    ...
    // La placa de la barra de título: con qué usuario se está trabajando.
    function pintarCuenta() {
        const e = win && win.querySelector('.sadeplus-cuenta');
        if (!e) return;
        e.textContent = USUARIO ? USUARIO : 'sin usuario';
        e.classList.toggle('sadeplus-sin-cuenta', !USUARIO || sesionVencida);
        e.title = !USUARIO
            ? 'No se pudo identificar el usuario: no se muestran ni se guardan etiquetas ni anotaciones, para no mezclar los datos de dos usuarios.'
            : sesionVencida
                ? 'La sesión del sistema venció. Recargue la página e ingrese nuevamente.'
                : 'Etiquetas y anotaciones del usuario ' + USUARIO + '. Cada usuario tiene las suyas: con otro usuario no se ve nada de éste.';
        if (sesionVencida && USUARIO) e.textContent = USUARIO + ' · sesión vencida';
    }
    ...
    //
    // El archivo exportado lleva anotaciones sobre actuaciones, y termina en
    // carpetas sincronizadas, en un pendrive o adjunto en un correo. Por eso
    // no sale como texto: tiene formato propio, binario, cifrado con una
    // contraseña del usuario. No se abre con un editor de texto, los
    // buscadores de escritorio y de la nube no leen su contenido, y sin la
    // contraseña no se recupera en ninguna parte.
    //
    // La contraseña se pone una sola vez y queda guardada en este equipo,
    // separada por usuario: acá no se la vuelve a pedir, ni al exportar ni al
    // importar. Al importar el archivo en otra computadora sí hay que
    // ponerla. Guardarla en el equipo no debilita lo que se busca proteger,
    // que es el archivo cuando sale: el almacén local ya contiene los mismos
    // datos y está detrás de la sesión de Windows.
    //
    // Cifrado AES-GCM con clave derivada por PBKDF2 (SHA-256, 250.000
    // vueltas), todo con las funciones propias del navegador. La sal y el
    // vector de inicialización son distintos en cada archivo y viajan con él.
    //
    //   bytes 0 a 5    la marca "SADE+" seguida de 0x01, que identifica el formato
    //   byte  6        versión del formato
    //   bytes 7 a 22   sal de la contraseña
    //   bytes 23 a 34  vector de inicialización
    //   resto          el contenido cifrado
    //
    // Las copias anteriores, que eran JSON en texto plano, se siguen
    // importando: se reconocen porque no empiezan con la marca.

    const MARCA_ARCHIVO = [0x53, 0x41, 0x44, 0x45, 0x2b, 0x01];   // "SADE+" 0x01
    const VERSION_ARCHIVO = 1;
    const VUELTAS_CLAVE = 250000;
    const EXT_ARCHIVO = '.sadeplus';
    const CLAVE_CONTRA = 'sade.plus.contra.v1';

    const bytesDe = (s) => new TextEncoder().encode(s);

    async function claveDesde(contra, sal) {
        const base = await crypto.subtle.importKey('raw', bytesDe(contra), 'PBKDF2', false, ['deriveKey']);
    ...
    const leerContra = () => {
        if (!USUARIO) return '';
        const v = leerAlmacen(CLAVE_CONTRA + '@' + USUARIO);
        return (v && typeof v.c === 'string') ? v.c : '';
    };
    const guardarContra = (c) => !!USUARIO &&
        escribirAlmacen(CLAVE_CONTRA + '@' + USUARIO, { c: String(c || ''), t: Date.now() });
    const hayContra = () => !!leerContra();

    const esArchivoCifrado = (b) => {
        if (!b || b.length < MARCA_ARCHIVO.length + 1 + 16 + 12 + 16) return false;
        return MARCA_ARCHIVO.every((x, i) => b[i] === x);
    };

    async function cifrarTexto(txt, contra) {
        if (!contra) throw new Error('Falta la contraseña de las copias.');
        const sal = crypto.getRandomValues(new Uint8Array(16));
        const iv = crypto.getRandomValues(new Uint8Array(12));
        const k = await claveDesde(contra, sal);
        const cifrado = new Uint8Array(await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, k, bytesDe(txt)));
        const p = MARCA_ARCHIVO.length;
        const out = new Uint8Array(p + 1 + sal.length + iv.length + cifrado.length);
        out.set(MARCA_ARCHIVO, 0);
        out[p] = VERSION_ARCHIVO;
        out.set(sal, p + 1);
        out.set(iv, p + 1 + sal.length);
        out.set(cifrado, p + 1 + sal.length + iv.length);
        return out;
    }

    async function descifrarTexto(bytes, contra) {
        const b = (bytes instanceof Uint8Array) ? bytes : new Uint8Array(bytes);
        if (!esArchivoCifrado(b)) return new TextDecoder().decode(b);   // copia anterior, en texto
        if (b[MARCA_ARCHIVO.length] !== VERSION_ARCHIVO) {
            throw new Error('El archivo fue generado por una versión más nueva de SADE+. Actualice la herramienta y vuelva a intentarlo.');
        }
        if (!contra) {
            throw new Error('El archivo está protegido con contraseña y en este equipo todavía no se cargó ninguna. Escríbala en "Contraseña de las copias" y vuelva a importar.');
        }
        const p = MARCA_ARCHIVO.length + 1;
        const k = await claveDesde(contra, b.subarray(p, p + 16));
        try {
            const claro = await crypto.subtle.decrypt({ name: 'AES-GCM', iv: b.subarray(p + 16, p + 28) }, k, b.subarray(p + 28));
            return new TextDecoder().decode(claro);
        } catch {
            throw new Error('La contraseña cargada en este equipo no abre ese archivo. Si se exportó con otra, escríbala en "Contraseña de las copias" y vuelva a importar.');
        }
    }
    ...
    async function exportarMarcas() {
        if (!USUARIO) { avisar(SIN_CUENTA); return false; }
        if (!hayContra()) {
            avisar('Antes de exportar hay que poner una contraseña para las copias, más abajo en este mismo panel.');
            return false;
        }
        refrescarMarcas();
        ...
        try {
            const bytes = await cifrarTexto(datos, leerContra());
            return bajar(bytes, 'SADE+-etiquetas-' + selloFecha() + EXT_ARCHIVO, 'application/octet-stream');
        } catch (e) {
            avisar('No se pudo generar la copia: ' + ((e && e.message) || 'error del navegador') + '.');
            return false;
        }
    }
    ...
    // y conserva las dos anotaciones cuando difieren. Un archivo de otro
    // usuario no se importa: mezclar los datos de dos personas es
    // justamente lo que la separación por usuario evita.
    function importarMarcas(texto) {
        if (!USUARIO) return SIN_CUENTA;
        let d;
        try { d = JSON.parse(texto); } catch { return 'El archivo no es una exportación de SADE+.'; }
        const formatoConocido = (f) => f === 'sade+/marcas' || f === 'ee-ayudante/marcas';
        if (!d || !formatoConocido(d.formato) || !d.filas || typeof d.filas !== 'object') {
            return 'El archivo no es una exportación de SADE+.';
        }
        if (d.usuario && d.usuario !== USUARIO) {
            return 'Ese archivo corresponde al usuario ' + d.usuario + ' y la sesión actual es la de ' +
                   USUARIO + '. No se importa, para no mezclar los datos de dos usuarios.';
        }
        refrescarMarcas();
        const mapa = {};                       // id de origen -> id acá
        for (const e of (Array.isArray(d.etiquetas) ? d.etiquetas : [])) {
            if (!e || !e.id || !e.nom) continue;
            const igual = MARCAS.etiquetas.find(x => norm(x.nom) === norm(e.nom));
            if (igual) { mapa[e.id] = igual.id; continue; }
            const id = crearEtiqueta(e.nom, colorDe(e.color).id);
            if (id) mapa[e.id] = id;
        }
        refrescarMarcas();
        let filas = 0;
        for (const k of Object.keys(d.filas)) {
            ...
            for (const x of (Array.isArray(orig.et) ? orig.et : [])) {
                const id = mapa[x] || x;
                if (etiquetaDe(id) && !et.includes(id)) et.push(id);
            }
            let nota = String(aca.nota || '');
            const otra = String(orig.nota || '').trim();
            // Una anotación que ya está incluida no se vuelve a agregar: así
            // importar dos veces el mismo archivo no duplica los textos.
            if (otra && !nota.includes(otra)) nota = nota.trim() ? nota + '\n\n' + otra : otra;
            if (et.length || nota.trim()) { MARCAS.filas[k] = { et, nota, t: Date.now() }; filas++; }
        }
        if (!guardarMarcas()) return 'No se pudo guardar lo importado.';
    ...
    /* --- usuario y sesión, en la barra de título --- */
    .sadeplus-tit .sadeplus-sesion{display:flex;align-items:center;gap:6px;flex:none;margin-left:6px}
    .sadeplus-cuenta{font:600 11px Consolas,monospace;background:#38485c;color:#fff;border-radius:10px;
      padding:2px 9px;white-space:nowrap;cursor:default}
    .sadeplus-cuenta.sadeplus-sin-cuenta{background:#c93b3b}
    .sadeplus-salir{height:24px;padding:0 9px;border:1px solid rgba(56,72,92,.35);background:transparent;
      color:#38485c;border-radius:4px;cursor:pointer;font:600 11px "Segoe UI",Arial,sans-serif;white-space:nowrap}
    .sadeplus-salir:hover{background:#38485c;color:#fff}
    #sadeplus.sadeplus-min .sadeplus-sesion{display:none}

    /* --- botones deshabilitados y avisos --- */
    .sadeplus-b:disabled,.sadeplus-acc button:disabled{opacity:.45;cursor:default}
    .sadeplus-b:disabled:hover{background:#fff6d1}
    .sadeplus-ini{border-color:#38485c;color:#38485c;font-weight:600}
    .sadeplus-datos p.sadeplus-alerta,.sadeplus-alerta{color:#8c2a1e;background:#fff0ee;border:1px solid #f1c6c0;
      border-radius:5px;padding:8px 11px}
    .sadeplus-contra{border:1px solid #e3d9b5;border-radius:5px;padding:0 10px;height:30px;
      font:13px Consolas,monospace;color:#38485c;width:320px;max-width:100%}

    /* --- aviso suelto --- */
    #sadeplus-cartel{position:fixed;right:18px;bottom:80px;z-index:2147482300;display:none;
      align-items:flex-start;gap:10px;max-width:min(460px,calc(100vw - 36px));
      background:#fffbea;color:#38485c;border:2px solid #facf00;border-left-width:6px;border-radius:7px;
      padding:10px 10px 10px 12px;box-shadow:0 10px 30px rgba(25,32,41,.35);
      font:13px/1.45 "Segoe UI",Arial,sans-serif}
    #sadeplus-cartel b{background:#ffdb2e;border-radius:4px;padding:0 6px;flex:none;font-size:12px;line-height:20px}
    #sadeplus-cartel span{flex:1}
    #sadeplus-cartel button{border:0;background:transparent;cursor:pointer;font-size:16px;line-height:18px;
      color:#69788a;padding:0 2px;flex:none}
    #sadeplus-cartel.error{border-color:#c93b3b}
    #sadeplus-cartel.ok{border-color:#26874a}
            ...
            <div class="sadeplus-about-in">
              ...
              <ul>
                ...
                <li><b>Etiquetas y anotaciones.</b> Marcas propias sobre cada actuación, que no
                    ...
                    la barra y se llevan a otra computadora exportándolas a un archivo cifrado.</li>
              ...
              <ul>
                ...
                <li>No consulta datos al servidor. Todo sale de lo que el navegador ya recibió. El
                    único pedido propio es el aviso de que la página sigue en uso, que no lee ni
                    escribe nada.</li>
                ...
                <li>No envía información a ningún lado. La configuración, las etiquetas y las
                    ...
                    cuando se exportan a un archivo cifrado.</li>
                <li>No completa ni confirma formularios del sistema.</li>
              </ul>
              ...
              <h3>Iniciar un documento en GEDO o una nota en CCOO</h3>
              <p>Los botones <b>＋ Documento GEDO</b> y <b>＋ Nota CCOO</b> de la barra abren, en una
                 pestaña nueva, el módulo que corresponde, y accionan allí el botón
                 <b>Inicio de Documento</b> del sistema, que en GEDO y en CCOO se llama igual. Queda a
                 la vista la ventana <b>Iniciar Producción de Documento</b>, con el tipo de documento
                 sin elegir: los datos se completan y se confirman a mano.</p>
              <p>El rótulo del botón se verifica dos veces antes de accionarlo, igual que con
                 Tramitar. Si el módulo pidiera ingresar de nuevo, ese botón no está: la herramienta lo
                 informa y no acciona nada. Un aviso amarillo con el nombre SADE+ dice, en la pestaña
                 nueva, cómo terminó el pedido.</p>

              <h3>Seguridad</h3>
              <ul>
                <li><b>Por usuario.</b> Las etiquetas, las anotaciones y la contraseña de las copias se
                    guardan separadas por usuario del sistema, que se lee de su encabezado. La barra de
                    título muestra con qué usuario se está trabajando. Si no puede identificarse, no se
                    muestra ni se guarda nada de eso.</li>
                <li><b>Varias pestañas.</b> Cada cambio vuelve a leer lo guardado y toca sólo la fila en
                    cuestión, de modo que dos pestañas no se pisan. Si las dos escribieron la misma
                    anotación, quedan los dos textos, separados por una línea.</li>
                <li><b>Copias cifradas.</b> El archivo exportado tiene formato propio (.sadeplus) y va
                    cifrado con una contraseña que se pone una sola vez en cada computadora. Sin ella no
                    se abre en ninguna parte. Las copias anteriores, en texto, se siguen importando. No se
                    importa un archivo de otro usuario.</li>
                <li><b>Pedidos a otra pestaña.</b> Cada pedido queda asignado a la pestaña que se abrió
                    para él, con su usuario y su módulo de destino. Si la página se recarga en medio, no
                    se repite ningún paso: se informa y se deja la verificación a quien usa la
                    herramienta.</li>
                <li><b>Abrir una actuación.</b> Un doble clic no la abre dos veces, y se comprueba que el
                    expediente haya aparecido.</li>
                <li><b>Sesión.</b> Mientras hay actividad (clic, tecla o rueda del mouse en los últimos
                    quince minutos) y la pestaña está a la vista, cada cinco minutos se le avisa al sistema
                    que la página sigue en uso. Con el equipo desatendido la sesión vence como siempre.
                    El botón <b>Cerrar sesión</b> de la barra de título sale con el control del propio
                    sistema; antes guarda la anotación que hubiera quedado sin guardar.</li>
              </ul>
    ...
    function ponerBusqueda(texto) {
        ...
        if (inp && inp.value !== filtro) inp.value = filtro;
    }
    ...
    /* Saltar a una pagina DENTRO DEL EXPEDIENTE.
       El paginador del SADE no es de SADE+, es de ZK. Pero el widget de
    ...
    // local = la paginacion la hace SADE+ sobre datos que ya tiene.
    ...
    //
    // Se excluye el encabezado común de los módulos y la propia herramienta:
    // en el encabezado está el control "Salir" del sistema, que cierra la
    // sesión y no tiene nada que ver con volver al listado.
    function controlVolverSADE() {
        const reVolver = /^\s*(volver|salir|cerrar)\s*$/i;
        const explicito = [...document.querySelectorAll(
            'button, a, input[type="button"], input[type="submit"], .z-button, .z-toolbarbutton')]
            .filter(x => vis(x) && !x.closest('.module-header, #sadeplus, #sadeplus-acerca, #sadeplus-cartel'))
            .find(x => reVolver.test(((x.innerText || '') + ' ' + (x.value || '')).trim()) ||
                       reVolver.test((x.getAttribute('title') || '').trim()));
    ...
    function pintar() {
        if (!win) return;
        pintarCuenta();
        if (MODO === 'gedo') { ajustarBarra(); pintarGedo(); avisoSinCuenta(); return; }
        if (MODO === 'expediente') { ajustarBarra(); pintarExpediente(); avisoSinCuenta(); return; }
        ajustarBarra();
        const L = filtradas();
        const porPag = CFG.porPagina;
        const paginas = Math.max(1, Math.ceil(L.length / porPag));
    ...
    // Sin usuario identificado, la barra de avisos lo dice de modo
    // permanente: las etiquetas y anotaciones no se muestran.
    function avisoSinCuenta() {
        if (USUARIO || !win) return;
        const av = win.querySelector('.sadeplus-aviso');
        if (!av) return;
        // No se tapa otro aviso que esté a la vista.
        if (av.style.display !== 'none' && av.textContent !== SIN_CUENTA) return;
        av.style.display = '';
        av.textContent = SIN_CUENTA;
    }
    ...
    function bloqueMarcas(clave) {
        ...
        return '<div class="sadeplus-marca-caja" data-marca="' + esc(clave) + '">' +
            ...
            '<textarea class="sadeplus-nota" data-base="' + esc(m.nota) + '" placeholder="Anotación privada sobre esta actuación. ' +
    ...
    // La contraseña de las copias. Se pone una vez y queda en este equipo,
    // separada por usuario; en otra computadora hay que volver a ponerla
    // para poder importar.
    let verContra = false;
    ...
    function contraHTML() {
        ...
        return '<h3>Contraseña de las copias</h3>' +
            '<p>El archivo exportado tiene formato propio de SADE+ (' + EXT_ARCHIVO + '), no es texto: ' +
            'no se abre con un editor ni lo leen los buscadores de escritorio o de la nube, y sin la ' +
            'contraseña no puede abrirse en ninguna parte. Es la protección del archivo cuando sale ' +
            'de esta computadora.</p>' +
            (puesta
              ? '<p><b>Contraseña puesta.</b> En esta computadora no se vuelve a pedir, ni para exportar ' +
                'ni para importar. Hay que escribirla en la otra computadora antes de importar allí el ' +
                'archivo.</p>'
              : '<p class="sadeplus-alerta">Todavía no hay contraseña, de modo que no se puede exportar. ' +
                'Conviene anotarla donde se guarden las demás claves: sin ella, el archivo no se abre ' +
                'en ninguna parte, tampoco aquí si se pierde esta computadora.</p>') +
            (puesta && !verContra
              ? '<div class="sadeplus-bts"><button class="sadeplus-b" data-acc="ver-contra">Ver o cambiar la contraseña</button></div>'
              : '<div class="sadeplus-et-nueva sadeplus-contra-caja">' +
                  '<input type="text" class="sadeplus-contra" maxlength="120" autocomplete="off" spellcheck="false" ' +
                    (puesta ? 'value="' + esc(leerContra()) + '"' : 'placeholder="Contraseña para las copias, de al menos 8 caracteres"') + '>' +
                  '<button class="sadeplus-b" data-acc="guardar-contra">Guardar la contraseña</button>' +
                  (puesta ? '<button class="sadeplus-b" data-acc="ocultar-contra">Listo</button>' : '') +
                '</div>');
    }
    ...
    function datosHTML() {
        ...
        return '<div class="sadeplus-datos-in">' +
            ...
            (USUARIO ? '' : '<p class="sadeplus-alerta">' + esc(SIN_CUENTA) + '</p>') +
            '<p>Las etiquetas y las anotaciones son datos propios, no del sistema: quedan ' +
            'guardados en este navegador, separados por usuario, y no se escriben en el ' +
            'expediente. ' + (USUARIO ? 'Usuario: <b>' + esc(USUARIO) + '</b>. ' : '') + 'Hoy hay <b>' +
            ...
            'de cada actuación y, si las anotaciones difieren, quedan las dos. Un archivo de otro ' +
            'usuario no se importa.</p>' +
            '<div class="sadeplus-bts">' +
              '<button class="sadeplus-b" data-acc="exportar"' + (hayContra() ? '' : ' disabled') +
                '>Exportar etiquetas y anotaciones</button>' +
    ...
    // Aviso de que se conservaron dos textos en una misma anotación.
    function avisarCombinada() {
        avisar('Otra pestaña había guardado un texto distinto en esta anotación mientras se escribía. ' +
               'Se conservaron los dos, separados por la línea "' + SEPARADOR_NOTA + '".', 12);
    }

    // Guarda la anotación de un bloque si tiene cambios pendientes.
    function guardarPendiente(caja) {
        const campo = caja && caja.querySelector('.sadeplus-nota');
        const boton = caja && caja.querySelector('.sadeplus-nota-guardar');
        if (!campo || !boton || !boton.classList.contains('sadeplus-pend')) return;
        const r = fijarMarca(caja.dataset.marca, { nota: campo.value, base: campo.dataset.base || '' });
        boton.classList.remove('sadeplus-pend');
        if (r.combinada) avisarCombinada();
    }
    ...
    function crear() {
        ...
        win.innerHTML = `
          <div class="sadeplus-tit">
            ...
            <span class="sadeplus-sesion">
              <span class="sadeplus-cuenta">sin usuario</span>
              <button class="sadeplus-salir" title="Cierra la sesión del sistema con su propio control de salida">Cerrar sesión</button>
            </span>
          ...
          <div class="sadeplus-barra">
            ...
            <button class="sadeplus-b sadeplus-ini" data-ini="gedo" title="Abre en una pestaña nueva el formulario de inicio de documento de GEDO. No completa ni confirma nada.">＋ Documento GEDO</button>
            <button class="sadeplus-b sadeplus-ini" data-ini="nota" title="Abre en una pestaña nueva el formulario de nueva nota de CCOO. No completa ni confirma nada.">＋ Nota CCOO</button>
          ...
          <input type="file" class="sadeplus-archivo" accept=".sadeplus,.json,application/json,application/octet-stream" style="display:none">
        ...
        win.querySelectorAll('.sadeplus-ini').forEach(b => b.addEventListener('click', () => {
            if (!b.disabled) iniciarEnPestana(b.dataset.ini, b);
        }));
        win.querySelector('.sadeplus-salir').addEventListener('click', cerrarSesion);
        ...
        const marcasClick = (ficha) => e => {
            ...
            if (bo && clave) {
                ...
                const caja = bo.closest('[data-marca]');
                const campo = caja && caja.querySelector('.sadeplus-nota');
                const r = fijarMarca(clave, { nota: '', base: campo ? (campo.dataset.base || '') : '' });
                refrescar();
                if (r.combinada) avisar('Otra pestaña había guardado un texto nuevo en esta anotación mientras tanto: ese texto se conservó.', 9);
                return;
            }

            const chip = e.target.closest('[data-et]');
            if (chip && clave) {
                // Si hay una anotación sin guardar en el mismo bloque, se
                // guarda antes: el redibujado la reemplazaría.
                guardarPendiente(chip.closest('[data-marca]'));
                alternarEtiqueta(clave, chip.dataset.et); refrescar(); return;
            }
        ...
        const marcasSalir = (ficha) => e => {
            ...
            const r = fijarMarca(clave, { nota: e.target.value, base: e.target.dataset.base || '' });
            boton.classList.remove('sadeplus-pend');
            if (r.combinada) {
                // Se muestra el texto combinado, que es lo que quedó guardado.
                if (fichaDe) verFicha(fichaDe); else pintar();
                avisarCombinada();
                return;
            }
            if (r.ok) {
                e.target.dataset.base = marcaDe(clave).nota || '';
        ...
        win.querySelector('.sadeplus-datos').addEventListener('click', e => {
            ...
            if (a === 'exportar') {
                acc.disabled = true;
                exportarMarcas().then(ok => {
                    acc.disabled = false;
                    if (ok) avisar('Copia exportada con la contraseña de este equipo. Conviene guardarla donde se guarden los demás papeles de trabajo.', 8);
                });
            }
            if (a === 'excel') exportarExcel();
            if (a === 'importar') {
                if (!USUARIO) { avisar(SIN_CUENTA); return; }
                archivo.click();
            }
            if (a === 'ver-contra') { verContra = true; panel('datos'); }
            if (a === 'ocultar-contra') { verContra = false; panel('datos'); }
            if (a === 'guardar-contra') {
                const campo = win.querySelector('.sadeplus-contra');
                const v = campo ? campo.value.trim() : '';
                if (v.length < 8) {
                    avisar('La contraseña tiene que tener al menos 8 caracteres.', 6);
                    if (campo) campo.focus();
                    return;
                }
                if (!guardarContra(v)) { avisar(USUARIO ? 'No se pudo guardar la contraseña.' : SIN_CUENTA); return; }
                verContra = false;
                panel('datos');
                avisar('Contraseña guardada en este equipo. Anótela donde guarde sus claves: sin ella, el archivo no se abre en ninguna parte.', 9);
            }
        });

        // El archivo se lee como bytes: el formato propio es binario, y una
        // copia anterior, que era texto, se decodifica al descifrar.
        archivo.addEventListener('change', () => {
            ...
            lector.onload = async () => {
                archivo.value = '';
                let texto;
                try { texto = await descifrarTexto(new Uint8Array(lector.result), leerContra()); }
                catch (err) { avisar((err && err.message) || 'No se pudo abrir el archivo.'); return; }
        ...
        const acciones = e => {
            ...
            if (bt && !bt.disabled) abrirActuacion(bt.dataset.tram, bt);
        };
    ...
    function arrancar() {
        ...
        // El encargo de las versiones anteriores ya no se usa.
        try { localStorage.removeItem(ENCARGO_VIEJO); } catch { /* sin almacenamiento */ }

        identificarUsuario();

        // En CCOO la herramienta sólo completa el inicio de una nota
        // encargado desde otra pestaña. No dibuja nada más.
        if (EN_CCOO) {
            cumplirEncargo();
            return;
        }
        ...
        crear();
        leer(); pintar(); pintarCuenta();

        // Si esta pestaña se abrió para un encargo (una actuación, un
        // documento de GEDO o una nota de CCOO), lo cumple y no muestra el
        // listado: lo que corresponde ver aquí es el sistema.
        ...
        // suspende mientras haya una ficha abierta. El usuario se vuelve a
        // mirar siempre: si cambió, se muestra lo suyo y nada del anterior.
        let firma = '';
        setInterval(() => {
            if (identificarUsuario()) {
                pintarCuenta();
                fichaDe = null; docAbierto = null; panel('lista'); pintar();
            }
